...
Rectangle{
    ...
    states: [
        State {
            name: "hover"
            PropertyChanges { target: controlButton; opacity:1}

        },
        State {
            name:"active"
            PropertyChanges {target: controlButton; opacity:1; x: posX-3; y: posY-3}
        },
        State {
            name: ""
            PropertyChanges {target: controlButton; opacity:0.5}
        }
    ]
    ...
    MouseArea{
        ...
        hoverEnabled: true
        onClicked: {
            if(controlButton.state === "hover") controlButton.state = "active";
            else if (controlButton.state === "active" ) controlButton.state = "hover";
            trigger();
        }
        onEntered: {
            if(controlButton.state === "") controlButton.state = "hover";
        }
        onExited: {
            if(controlButton.state == "hover") controlButton.state = "";
        }
    }
}
